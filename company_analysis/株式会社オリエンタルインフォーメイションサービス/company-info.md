# 株式会社オリエンタルインフォーメイションサービス (Oriental Information Service Corporation)

## 基本情報
- **会社名**: 株式会社オリエンタルインフォーメイションサービス
- **英語名**: Oriental Information Service Corporation
- **ウェブサイト**: https://www.ois-yokohama.co.jp/
- **所在地**: 220-6118 横浜市西区みなとみらい2-3-3 クイーンズタワーB18F
- **事業内容**: 受託開発ソフトウェア、組み込みソフトウェア

## 会社概要

**事業内容**: 受託開発ソフトウェア、組み込みソフトウェア

**事業内容詳細**: 横浜・みなとみらいに拠点を構え、大手メーカーやエンドユーザーを中心にWeb・スマホアプリ開発や組込系ソフト開発を行っています。

## 採用情報

**採用区分**: 新卒採用 | 中途採用 | 常駐・派遣

**特徴**: 横浜・みなとみらいに拠点を構え、大手メーカーやエンドユーザーを中心にWeb・スマホアプリ開発や組込系ソフト開発を行っています。

## 会社説明

株式会社オリエンタルインフォーメイションサービスは、横浜市西区みなとみらいに拠点を置くソフトウェア開発会社です。大手メーカーやエンドユーザーを中心にWeb・スマホアプリ開発や組込系ソフト開発を行っています。

## 会社特徴

- 横浜・みなとみらいに拠点を構える
- 大手メーカーやエンドユーザー向けの開発を手掛ける
- Web・スマホアプリ開発に強み
- 組込み系ソフト開発も実施
- 受託開発ソフトウェア事業を展開
- 新卒、中途採用、常駐・派遣の採用区分がある

## 研究状況

**Current Limitations**: 
- Web search functionality is currently disabled due to MiniMax API key configuration issues
- Direct website access may be limited
- Cannot perform detailed company research at this time

**Next Steps**:
- Wait for web search functionality restoration
- Research company details from official website
- Search for company mentions in news and media
- Research employee profiles for LinkedIn and Twitter links

---

*This is a placeholder file. Detailed research will be conducted when web search functionality becomes available.*